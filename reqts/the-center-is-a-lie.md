...
- **Trust is ~orthogonal to ideology — the center is *never the lowest* (ANES CDF, mean trust by ideology):** the lowest-trust group is always the **out-party** (right under Dem presidents 2012/2016/1996; left under Rep presidents 2020/2004), and **pooled 1972–2024 the center is the *highest*-trust** (0.30 vs Left 0.26 / Right 0.27). So **"add trust → the center empties out" is false**, and any single year is incumbent-tilted. **Figs 1–2 should be built from a multi-cycle composite** (partisan tilt cancels). The honest Fig-2 claim: adding trust reveals a *second axis that barely tracks left-right*, on which *almost everyone sits low* — the 1-D line is wrong not about the center but because it omits the dimension that moves voters.
